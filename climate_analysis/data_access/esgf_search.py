...
def _choose_access(url_field: str) -> Optional[FileAccess]:
    ...
    Preference order: OPeNDAP -> HTTPServer -> first available.
    """

    entries = url_field.split("|")
    ...
    preferred = ("OPENDAP", "HTTPServer")
    for service in preferred:
        for url, mime, svc in triples:
            if svc == service:
                return FileAccess(url=url, service=svc, mime=mime)
    if triples:
...
def search_files(cfg: ESGFSearchConfig, model: str, experiment: str) -> List[FileRecord]:
    ...
    for _ in range(cfg.max_pages):
        ...
        resp = requests.get(url, params=params_with_offset, timeout=30)
